...
- **Large files (P2)** — ~20 non-test source files exceed 250 LOC. Top offenders: `ntgw-ir/src/lib.rs` (1103), `ntgw-ir/src/snapshot.rs` (1082), `ntgw-ir/src/snapshot/backend_resolution.rs` (1063), `ntgw-http/src/proxy.rs` (1000). The 250-line soft cap is aspirational; progressive splitting is tracked in the platform TODO. No immediate action without a dedicated splitting plan.
