...
fn stream_listener_plan_ignores_non_topology_listener_changes() {
    ...
    let before = build_listener_plan(&ntgw_ir::Snapshot {
        listeners: vec![base],
        ..ntgw_ir::Snapshot::default()
    });
    let after = build_listener_plan(&ntgw_ir::Snapshot {
...
fn stream_listener_plan_changes_when_bind_changes() {
    ...
    let tcp_plan = build_listener_plan(&ntgw_ir::Snapshot {
        listeners: vec![tcp],
        ..ntgw_ir::Snapshot::default()
    })
    .expect("tcp plan");
    let moved_plan = build_listener_plan(&ntgw_ir::Snapshot {
        listeners: vec![moved],
        ..ntgw_ir::Snapshot::default()
    })
    .expect("moved plan");
...
fn build_listener_plan_ignores_tls_passthrough_listeners() {
    ...
    assert!(
        plan.is_none(),
        "tls passthrough listeners now belong to ntgw-shared-tls"
    );
}
