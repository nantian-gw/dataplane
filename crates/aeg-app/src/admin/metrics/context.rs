...
use super::super::{
    ...
    summary::{
        ...
        snapshot_requires_tls_runtime, snapshot_session_persistence_usage, CurrentSnapshotState,
...
use aeg_observability::{
    snapshot_access_log_writers, snapshot_process, AccessLogWriterSnapshot,
    AdminRequestStatsSnapshot, HttpCircuitBreakerSnapshot, HttpRateLimitSnapshot, OverloadSnapshot,
    ProcessSnapshot, RetryBudgetSnapshot, RuntimeStatsSnapshot, TrafficSnapshot,
    UdpSessionSnapshot,
};
use aeg_xds::ClientStatsSnapshot;
...
impl MetricsContext {
    pub(super) fn from_state(state: &AppState) -> Self {
        ...
        let http3_available = u64::from(aeg_http::http3_available());
        let http3_configured = u64::from(config.http3_configured);
        let http3_enabled = u64::from(config.http3_configured && aeg_http::http3_available());
