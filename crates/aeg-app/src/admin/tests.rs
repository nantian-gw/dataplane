pub(super) use aeg_ir::{
    ...
    StreamRule, TlsRouteMode, PASSIVE_EJECTION_CONSECUTIVE_FAILURES,
};
pub(super) use aeg_observability::{
...
pub(super) use aeg_xds::ClientStats;
pub(super) use axum::{
    body::Body,
    http::header,
...
pub(super) use super::{
    build_listener_runtime_status, build_router, build_summary_value,
    collect_listener_runtime_statuses, documented_route_contracts, filter_listeners, find_backend,
    find_listener, find_route, render_metrics, AdminRouteContract, AdminRuntimeConfig, AppState,
    BackendListQuery, ListenerListQuery, RouteListQuery,
};
...
fn fixture_snapshot() -> Snapshot {
    Snapshot {
        ...
        http_routes: vec![aeg_ir::HttpRoute {
            name: "web".to_string(),
...
fn build_state_with_parts(
    ...
    runtime: aeg_observability::SharedRuntimeStats,
    xds: aeg_xds::SharedClientStats,
) -> AppState {
    AppState {
...
fn temp_token_path(prefix: &str) -> PathBuf {
    let unique = SystemTime::now()
        ...
        .expect("system time")
        .as_nanos();
    std::env::temp_dir().join(format!("aeg-app-{prefix}-{unique}.token"))
}
