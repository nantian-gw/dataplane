...
#[tokio::test]
async fn listener_status_endpoint_filters_by_has_ever_failed() {
    ...
    state
        .runtime
        .observe_http_listener_reload_result("v2", &["clean".to_string()], &[], &[]);
    let app = build_router(state);

    let response = app
        .oneshot(
            Request::builder()
                .uri("/v1/listener-statuses?hasEverFailed=true")
    ...
    assert_eq!(payload[0]["listener_has_ever_failed"], true);
    assert_eq!(payload[1]["name"], "failed");
    assert_eq!(payload[1]["listener_has_ever_failed"], true);
}
