...
#[tokio::test]
async fn listener_status_endpoint_filters_by_attention_required() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri("/v1/listener-statuses?attentionRequired=true")
    ...
    assert_eq!(payload[0]["listener_attention_required"], true);
    assert_eq!(payload[1]["name"], "stale");
    assert_eq!(payload[1]["listener_attention_required"], true);
}
