...
async fn traffic_view_returns_observed_flow_stats() {
    ...
    assert!(payload["nodes"].as_array().is_some_and(|items| {
        items
            .iter()
    ...
    assert!(payload["edges"].as_array().is_some_and(|items| {
        items
            .iter()
    ...
    assert!(payload["edges"].as_array().is_some_and(|items| {
        items.iter().any(|item| {
            item["edge_id"] == "edge:route:HTTPRoute:default/web:backend:default/api:80"
        })
    }));
}
