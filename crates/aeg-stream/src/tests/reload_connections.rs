...
async fn tcp_route_reload_preserves_existing_connection() -> Result<()> {
    ...
    let snapshot = aeg_ir::Snapshot::shared();
    *snapshot.write() = tcp_snapshot("v1", listener.clone(), "upstream-a", upstream_a_addr);
    let updates = aeg_ir::SnapshotSignal::shared();
    ...
    let runtime_task = tokio::spawn(run(
        ...
        runtime_stats.clone(),
        traffic,
        aeg_observability::UdpSessionStats::shared(),
        overload,
        shutdown_rx,
    ));
...
fn tcp_snapshot(
    ...
    let mut snapshot = aeg_ir::Snapshot {
        id: version.to_string(),
        listeners: vec![listener.clone()],
        stream_routes: vec![aeg_ir::StreamRoute {
            name: "tcp-route".to_string(),
            namespace: "default".to_string(),
            kind: "ROUTE_KIND_TCP".to_string(),
            parent_refs: Vec::new(),
            rules: vec![aeg_ir::StreamRule {
                name: String::new(),
                matches: vec![aeg_ir::StreamMatch {
                    port: listener.port,
                    sni_hostname: String::new(),
                    mode: aeg_ir::TlsRouteMode::default(),
                }],
                backend_refs: vec![aeg_ir::BackendRef {
                    namespace: "default".to_string(),
                    name: backend_name.to_string(),
                    port: upstream_addr.port() as u32,
                    ..aeg_ir::BackendRef::default()
                }],
            }],
...
async fn wait_for_stream_listener_attempt(
    runtime_stats: aeg_observability::SharedRuntimeStats,
    listener: &str,
    version: &str,
    attempts: u64,
    max_wait: Duration,
