...
use aeg_ir::{SelectedBackend, SelectedBackendRuntimeIds, SharedSnapshot};
use aeg_observability::{
    current_timestamp, write_access_log, AccessLogOptions, AccessLogRecord, SharedTrafficStats,
    TcpAdmissionController, TrafficObservationRef, TrafficRuntimeIds,
    UPSTREAM_CONNECT_LATENCY_MS_BUCKET_COUNT,
};
...
use crate::{
    ...
    socket_addr,
    traffic::{stream_route_kind_label, ZERO_UPSTREAM_CONNECT_LATENCY_MS_BUCKETS},
};
...
fn record_tcp_connect_failure(traffic: &SharedTrafficStats, failure: TcpConnectFailure<'_>) {
    let upstream_connect_latency_ms_buckets = {
        ...
        let bucket =
            aeg_observability::upstream_connect_latency_ms_bucket_index(failure.connect_latency_ms);
        buckets[bucket] = 1;
        buckets
    };
    traffic.observe_ref(TrafficObservationRef {
