...
fn prefers_grpc_persistent_session_backend_over_weighted_selection() {
    ...
    let selected = snapshot
        .select_grpc_backend_with_session_resolver(&request, |_| {
            Some(session_target("default/greeter-b:9091", "10.0.0.21", 9091))
        })
        .expect("backend");

    assert_eq!(selected.backend_name, "default/greeter-b:9091");
    assert_eq!(selected.backend.address, "10.0.0.21");
}
