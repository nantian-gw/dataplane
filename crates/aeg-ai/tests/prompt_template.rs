use aeg_ai::format::ir::{AIRequest, AIRole};
use aeg_ai::prompt_template::{PromptInjector, PromptTemplate};
...
fn inject_system_prompt() {
    ...
    if let aeg_ai::format::ir::AIContent::Text(ref t) = request.messages[0].content {
...
fn inject_few_shot_examples() {
    ...
    if let aeg_ai::format::ir::AIContent::Text(ref t) = request.messages[1].content {
        assert_eq!(t, "What is 2+2?");
    }
    if let aeg_ai::format::ir::AIContent::Text(ref t) = request.messages[2].content {
        assert_eq!(t, "4");
    }
}
...
fn inject_preserves_existing_messages() {
    ...
    let mut request = make_request("gpt-4");
    request.messages.push(aeg_ai::format::ir::AIMessage {
        role: AIRole::User,
        content: aeg_ai::format::ir::AIContent::Text("real question".into()),
        name: None,
        tool_calls: Vec::new(),
        tool_call_id: None,
    });
