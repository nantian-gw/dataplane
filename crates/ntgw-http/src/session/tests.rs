...
use super::{
    FileSecretSource, SecretSource, SessionManager, SessionPersistenceOptions, cookie_path,
    longest_literal_cookie_path,
};
use ntgw_ir::{BackendEndpoint, MatchedHttpPath, SelectedBackend, SessionPersistence};
...
fn temp_secret_path(prefix: &str) -> PathBuf {
    let unique = SystemTime::now()
        ...
        .expect("system time")
        .as_nanos();
    std::env::temp_dir().join(format!("ntgw-http-{prefix}-{unique}.key"))
}
