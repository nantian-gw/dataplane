#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn http1_large_content_length_request_body_is_forwarded_intact() {
    install_rustls_provider();
    let upstream_listener = TcpListener::bind("127.0.0.1:0")
        .await
        .expect("upstream bind");
    let upstream_addr = upstream_listener.local_addr().expect("upstream addr");
    let gateway_port = free_tcp_port();
    let snapshot = simple_http_snapshot(
        gateway_port,
        "/upload-large",
        upstream_addr.port() as u32,
        "HTTP",
    );
    let runtime = RuntimeOptions {
        enable_ipv6: false,
        ..RuntimeOptions::default()
    };
    let plan = build_listener_plan(&snapshot.read(), &runtime, None).expect("plan");
    let server = start_server(
        plan,
        snapshot.clone(),
        runtime,
        AccessLogOptions {
            enabled: false,
            ..AccessLogOptions::default()
        },
        SessionPersistenceOptions::build(None, None).expect("session options"),
        SharedTrafficStats::shared(),
    )
    .expect("start server");

    let payload = vec![b'a'; 512 * 1024];
    let upstream_payload = payload.clone();
    let upstream = tokio::spawn(async move {
        let (mut stream, _) = upstream_listener.accept().await?;
        let request = read_http_headers(&mut stream).await?;
        assert!(request.starts_with("POST /upload-large HTTP/1.1\r\n"));
        let expected_length = upstream_payload.len().to_string();
        assert_eq!(
            header_value(&request, "content-length"),
            Some(expected_length)
        );
        let body = read_http_body(&mut stream, &request).await?;
        assert_eq!(body, upstream_payload);
        stream
            .write_all(b"HTTP/1.1 200 OK\r\nContent-Length: 5\r\n\r\nlarge")
            .await?;
        Ok::<(), anyhow::Error>(())
    });

    wait_for_listener(gateway_port).await;

    let result = async {
        let mut client = TcpStream::connect(("127.0.0.1", gateway_port)).await?;
        let headers = format!(
            "POST /upload-large HTTP/1.1\r\nHost: example.com\r\nContent-Length: {}\r\n\r\n",
            payload.len()
        );
        client.write_all(headers.as_bytes()).await?;
        client.write_all(&payload).await?;
        let response = read_http_response(&mut client).await?;
        assert!(response.starts_with("HTTP/1.1 200"));
        assert!(response.ends_with("\r\n\r\nlarge"));
        Ok::<(), anyhow::Error>(())
    }
    .await;

    stop_server(server);
    result.expect("large content-length client flow");
    upstream
        .await
        .expect("upstream task")
        .expect("upstream result");
}
