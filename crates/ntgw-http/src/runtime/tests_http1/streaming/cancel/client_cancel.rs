...
async fn streaming_http_client_cancel_does_not_retry_to_next_backend() {
    ...
    let log_path = temp_log_path("streaming-http-client-cancel");
    ...
    let streaming = tokio::spawn(async move {
        ...
        let mut eof = [0; 1];
        let _ = timeout(Duration::from_millis(500), stream.read(&mut eof)).await;
        Ok::<(), anyhow::Error>(())
    });
    ...
    result.expect("streaming client cancel flow");
    streaming
        .await
    ...
    assert!(
        ...
        "client-cancelled stream must not be retried to the next backend"
    );
    ...
    assert!(
        ...
        "client cancel should be logged as downstream close, got: {log_contents}"
    );
