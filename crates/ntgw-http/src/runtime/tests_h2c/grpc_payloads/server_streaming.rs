...
async fn grpc_server_streaming_response_is_forwarded_over_h2c() {
    ...
    let first_frame = grpc_data_frame(b"first");
    let second_frame = grpc_data_frame(b"second");
    let first_frame_upstream = first_frame.clone();
    let second_frame_upstream = second_frame.clone();
    let upstream = tokio::spawn(async move {
        ...
        send.send_data(bytes::Bytes::from(second_frame_upstream), false)?;
    ...
    let result = async {
        ...
        let collected = read_h2_body_stream(&mut body).await?;
        assert_eq!(collected, [first_frame, second_frame].concat());
        let trailers = body.trailers().await?.expect("grpc trailers");
        assert_eq!(
            trailers
                .get("grpc-status")
                .and_then(|value| value.to_str().ok()),
    ...
    stop_server(server);
    result.expect("grpc client flow");
    upstream
        .await
