...
async fn grpc_weighted_backends_reselect_per_concurrent_stream_on_one_downstream_connection() {
    ...
    let snapshot = weighted_grpc_h2c_snapshot(
        gateway_port,
    ...
    let result = async {
        ...
        for response in responses {
            ...
            assert_eq!(
                status,
                StatusCode::OK,
                "unexpected concurrent gRPC status {status}; headers={headers:?}; traffic={:?}",
                traffic.snapshot()
            );
    ...
    stop_server(server);
    result.expect("grpc concurrent weighted client flow");
