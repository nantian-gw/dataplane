...
pub(crate) fn run_snapshot_read_rwlock(
    ...
    let shared = aeg_ir::Snapshot::shared();
...
fn apply_runtime_index_mutation(snapshot: &mut aeg_ir::Snapshot, mutation: RuntimeIndexMutation) {
        ...
        RuntimeIndexMutation::Route => {
            if let Some(route) = snapshot.http_routes.first_mut() {
                ...
                if let Some(rule) = route.rules.first_mut() {
                    if let Some(route_match) = rule.matches.first_mut() {
