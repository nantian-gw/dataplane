...
pub(crate) fn run_header_filter_chain(
    iterations: u32,
    config: aeg_http::bench::FilterChainBenchConfig,
) -> Result<ScenarioReport> {
    let fixture = aeg_http::bench::FilterChainFixture::build(config);
...
pub(crate) fn run_request_meta_header_heavy(
    iterations: u32,
    config: aeg_http::bench::RequestMetaBuildBenchConfig,
) -> Result<ScenarioReport> {
    let fixture = aeg_http::bench::RequestMetaBuildFixture::build(config)?;
    let before = sample_resources();
    let mut durations = Vec::with_capacity(iterations as usize);
    let mut last_step = None;
...
pub(crate) fn run_request_view_header_heavy(
    iterations: u32,
    config: aeg_http::bench::RequestViewBuildBenchConfig,
) -> Result<ScenarioReport> {
    let fixture = aeg_http::bench::RequestViewBuildFixture::build(config)?;
    let before = sample_resources();
    let mut durations = Vec::with_capacity(iterations as usize);
...
pub(crate) fn run_request_fast_path_selection(
    ...
    let fixture = aeg_http::bench::FastPathSelectionFixture::build(config)?;
    let before = sample_resources();
    let mut durations = Vec::with_capacity(iterations as usize);
...
pub(crate) fn run_session_persistence(
    iterations: u32,
    config: aeg_http::bench::SessionBenchConfig,
) -> Result<ScenarioReport> {
    let fixture = aeg_http::bench::SessionBenchFixture::build(config)?;
    let before = sample_resources();
    let mut durations = Vec::with_capacity(iterations as usize);
    let mut last_step = None;
...
pub(crate) fn run_access_log_write_path(
    iterations: u32,
    config: aeg_observability::bench::AccessLogBenchConfig,
) -> Result<ScenarioReport> {
...
pub(crate) fn run_access_log_disabled_path(
    ...
    run_access_log_path(
        ...
        iterations,
        aeg_observability::bench::AccessLogBenchConfig {
            enabled: false,
            ..config
        },
    )
...
fn run_access_log_path(
    ...
    let fixture = aeg_observability::bench::AccessLogFixture::build(config);
    let before = sample_resources();
    let mut durations = Vec::with_capacity(iterations as usize);
    let mut last_step = None;
