...
pub(crate) fn run_tls_asset_rotation(
    iterations: u32,
    config: aeg_http::runtime_bench::TlsRotationBenchConfig,
) -> Result<ScenarioReport> {
    let fixture = aeg_http::runtime_bench::TlsRotationFixture::build(config);
...
pub(crate) async fn run_high_frequency_apply(
    iterations: u32,
    config: aeg_xds::bench::ApplyBenchConfig,
) -> Result<ScenarioReport> {
    let fixture = aeg_xds::bench::ReloadBench::new(config);
    let before = sample_resources();
    let mut durations = Vec::with_capacity(iterations as usize);
