...
pub(crate) fn run_stream_tcp_buffer_matrix(
    iterations: u32,
    config: aeg_stream::bench::StreamBenchConfig,
) -> Result<ScenarioReport> {
    let fixture = aeg_stream::bench::StreamTcpBufferMatrixFixture::build(config);
...
pub(crate) fn run_stream_udp_dispatcher_distribution(
    ...
    let fixture = aeg_stream::bench::StreamUdpDistributionFixture::build(config);
    let before = sample_resources();
    let mut durations = Vec::with_capacity(iterations as usize);
    let mut last_step = None;
...
pub(crate) fn run_stream_udp_payload_copy(
    ...
    let fixture = aeg_stream::bench::StreamUdpPayloadCopyFixture::build(config);
    let before = sample_resources();
    let mut durations = Vec::with_capacity(iterations as usize);
    let mut last_step = None;
