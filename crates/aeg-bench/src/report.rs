...
use crate::scenarios::{
    ...
    run_grpc_route_selection, run_header_filter_chain, run_high_frequency_apply,
    run_http_capacity_matrix, run_http_route_selection, run_large_snapshot_switch,
    run_last_good_fallback, run_request_fast_path_selection, run_request_meta_header_heavy,
...
pub(crate) struct BenchConfig {
    ...
    pub(crate) snapshot: aeg_ir::bench::SnapshotBenchConfig,
    pub(crate) tls_rotation: aeg_http::runtime_bench::TlsRotationBenchConfig,
    pub(crate) xds_apply: aeg_xds::bench::ApplyBenchConfig,
    pub(crate) request_meta: aeg_http::bench::RequestMetaBuildBenchConfig,
    pub(crate) filter_chain: aeg_http::bench::FilterChainBenchConfig,
    pub(crate) session_persistence: aeg_http::bench::SessionBenchConfig,
    pub(crate) access_log: aeg_observability::bench::AccessLogBenchConfig,
    pub(crate) traffic_stats: aeg_observability::bench::TrafficStatsBenchConfig,
    pub(crate) http_capacity: aeg_http::runtime_bench::HttpCapacityMatrixBenchConfig,
    pub(crate) stream: aeg_stream::bench::StreamBenchConfig,
}
...
pub(crate) async fn build_report(config: BenchConfig) -> Result<BenchReport> {
    ...
    Ok(BenchReport {
        ...
        allocator: aeg_allocator::selected_allocator().to_string(),
        config,
        scenarios,
        comparisons,
    })
...
pub(crate) fn sample_resources() -> ResourceSample {
    ...
    if let Ok(stat) = fs::read_to_string("/proc/self/stat") {
        if let Some((user_ticks, system_ticks)) = parse_proc_stat_cpu_ticks(&stat) {
