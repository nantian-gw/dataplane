...
use report::{build_report, BenchConfig};
...
async fn main() -> Result<()> {
    ...
    let snapshot_config = aeg_ir::bench::SnapshotBenchConfig {
    ...
    let tls_config = aeg_http::runtime_bench::TlsRotationBenchConfig {
        listeners: cli.tls_listeners,
        ca_bundle_variants: cli.tls_ca_bundle_variants,
    };
    let xds_config = aeg_xds::bench::ApplyBenchConfig::default();
    let request_meta_config = aeg_http::bench::RequestMetaBuildBenchConfig::default();
    let filter_chain_config = aeg_http::bench::FilterChainBenchConfig::default();
    let session_config = aeg_http::bench::SessionBenchConfig::default();
    let access_log_config = aeg_observability::bench::AccessLogBenchConfig::default();
    let traffic_stats_config = aeg_observability::bench::TrafficStatsBenchConfig::default();
    let http_capacity_config = aeg_http::runtime_bench::HttpCapacityMatrixBenchConfig::default();
    let stream_config = aeg_stream::bench::StreamBenchConfig::default();
