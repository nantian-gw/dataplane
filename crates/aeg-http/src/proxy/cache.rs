...
use aeg_ir::{BackendTlsValidation, SessionPersistence};
...
impl ClientCertCacheKey {
    pub(crate) fn new(secret: &aeg_ir::SecretMaterial) -> Self {
        Self {
...
fn digest_subject_alt_names(items: &[aeg_ir::BackendSubjectAltName]) -> [u8; 32] {
    let mut digest = Sha256::new();
    for item in items {
...
impl<'a> SessionResolutionCache<'a> {
    ...
    ) -> Option<aeg_ir::PersistentSessionTarget> {
