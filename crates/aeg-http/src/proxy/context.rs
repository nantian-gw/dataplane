...
use aeg_ai::filter::AIContext;
use aeg_ir::{
    BackendEndpoint, EndpointRuntimeHandle, RouteKind, SelectedBackend, SelectedBackendRuntimeIds,
    SharedSnapshot,
};
use aeg_observability::{
    epoch_millis, upstream_connect_latency_ms_bucket_index, AccessLogOptions, HttpAdmissionPermit,
    HttpCircuitBreakerPermit, SharedTrafficStats, TrafficTopology,
...
pub(crate) struct FastSelectedBackendState {
    pub(crate) selected: aeg_ir::CompiledSelectedHttpBackend,
}
