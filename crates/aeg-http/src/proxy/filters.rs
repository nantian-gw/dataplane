use aeg_ir::SessionPersistence;
use aeg_wasm::WasmError;
use bytes::Bytes;
...
pub(crate) async fn do_request_filter(
    ...
    let Some((route, mirrors, frontend_client_certificate_requirement)) = http_selection else {
            ...
            fallback_selection
        {
            ...
            if let (Some(ref route_ns), Some(ref route_name)) = (&route_ns, &route_name) {
                if try_cache_hit(
                    proxy,
                    session,
                    ctx,
    ...
    if let Some(ref ai_filter) = proxy.ai_filter {
        ...
        match ai_filter.pre_process(ctx.path.as_str(), &body, None).await {
            ...
            Err(e) => {
                if matches!(e, aeg_ai::error::AIError::UnsupportedFormat(_)) {
