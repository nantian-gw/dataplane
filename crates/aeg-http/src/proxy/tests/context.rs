...
use aeg_observability::{upstream_connect_latency_ms_bucket_index, SharedTrafficStats};
...
fn sample_runtime_handle(address: &str) -> aeg_ir::EndpointRuntimeHandle {
    let snapshot = sample_runtime_snapshot();
    snapshot.endpoint_runtime_handle(&sample_selected_backend(address, "default/orders:8443"))
}

fn sample_traffic_topology(backend_name: &str) -> aeg_observability::TrafficTopology {
    aeg_observability::TrafficTopology::from_parts(
        "listener",
        "Http",
        "default",
        "orders",
