...
use aeg_observability::{
    current_timestamp, emit_access_log, render_access_log, resolve_access_log_write_options,
    AccessLogOptions, AccessLogRecord, AccessLogSampleKey, SharedTrafficStats,
    TrafficObservationRef, TrafficRuntimeIds,
};
use tracing::error;

use super::context::{
    clear_completed_request_context, route_kind_name, saturating_latency_ms, RequestContext,
};
...
mod tests {
    ...
    use aeg_observability::shutdown_access_log_writer;
    ...
    fn observe_completed_request_emits_runtime_ids_in_access_log() {
        ...
        observe_completed_request(
            &AccessLogOptions {
                ...
                mode: aeg_observability::AccessLogMode::Json,
    ...
    fn observe_completed_request_prefers_cached_local_response_topology() {
        ...
        let mut ctx = RequestContext {
            ...
            local_response_traffic_topology: Some(Arc::new(
                aeg_observability::TrafficTopology::unmatched("default/gw/http"),
            )),
            ..RequestContext::default()
        };
    ...
    fn temp_log_path(prefix: &str) -> std::path::PathBuf {
        let unique = SystemTime::now()
            ...
            .expect("system time")
            .as_nanos();
        std::env::temp_dir().join(format!("aeg-http-proxy-{prefix}-{unique}.log"))
    }
