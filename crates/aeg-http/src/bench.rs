...
use crate::{
    ...
    proxy::{
        ...
        start_request_span_from_header, RequestContext, RequestView,
    },
...
impl FastPathSelectionFixture {
    ...
    pub fn select(&self) -> Result<aeg_ir::CompiledSelectedHttpBackend> {
...
impl SessionBenchFixture {
    pub fn build(config: SessionBenchConfig) -> Result<Self> {
        ...
        Ok(Self {
            manager,
            policy: SessionPersistence {
                session_name: "aeg-bench-session".to_string(),
