...
use aeg_ai::wasm_filter::WasmPluginFilter;
...
use self::context::{
    assign_ctx_string, cache_fast_selected_backend_state, cache_http_route_context,
    ...
    selected_backend_is_transport_retry_excluded, store_admission_permit, HttpRouteContextFields,
...
use self::external_auth::{
    apply_external_auth_response_headers, external_auth_filter, run_external_auth,
...
pub(crate) use self::request::{
    build_request_meta_from_header_with_port, capture_request_context_from_view,
    fast_path_request_from_header, start_request_span_from_header,
    start_request_span_from_header_if_enabled, RequestView,
};
use self::responses::{
...
use self::selection::{
    selected_backend_config_cached, selected_backend_config_cached_for_fast_path,
    selected_backend_from_http_route, SelectedBackendConfigCache,
};
...
pub(crate) struct InitialFastPathSelection {
    pub(super) selected: aeg_ir::CompiledSelectedHttpBackend,
...
pub struct GatewayProxy {
    ...
    pub(crate) ai_filter: Option<Arc<aeg_ai::filter::AIGatewayFilter>>,
}
...
impl GatewayProxy {
    ...
    pub fn new(
        ...
        ai_filter: Option<Arc<aeg_ai::filter::AIGatewayFilter>>,
    ) -> Self {
        Self {
            snapshot,
            access_log,
...
impl ProxyHttp for GatewayProxy {
        ...
        Self::CTX: Send + Sync,
    {
        ...
        if let Some(selected) = ctx.selected_backend.as_ref() {
            if let Some(policy) = selected.session_persistence.as_ref() {
        ...
        Self::CTX: Send + Sync,
    {
        ...
        if let Some(http_cache) = ctx.http_cache.0.as_mut() {
            if let Some(miss_handler) = http_cache.miss_handler() {
