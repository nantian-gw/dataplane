        ...
        http_routes: vec![HttpRoute {
            ...
            rules: vec![HttpRule {
                ...
                filters: vec![Filter {
                    ...
                    external_auth: Some(aeg_ir::ExternalAuthFilter {
                        ...
                        http: aeg_ir::ExternalHTTPAuthConfig {
                            path: "/auth".to_string(),
                        ...
                        grpc: aeg_ir::ExternalGRPCAuthConfig::default(),
            ...
            rules: vec![HttpRule {
                ...
                filters: vec![Filter {
                    ...
                    external_auth: Some(aeg_ir::ExternalAuthFilter {
                        ...
                        grpc: aeg_ir::ExternalGRPCAuthConfig {
                            allowed_headers: allowed_headers.into_iter().map(str::to_string).collect(),
                        },
                        ..aeg_ir::ExternalAuthFilter::default()
                    }),
                    ..Filter::default()
                }],
                backend_refs: vec![BackendRef {
...
fn enable_external_auth_forward_body(snapshot: &aeg_ir::SharedSnapshot, max_size: u32) {
...
fn external_auth_backend_tls_validation_in_snapshot() {
    use aeg_ir::{BackendTlsValidation, Snapshot};

    let auth_port: u32 = 8443;
    let snapshot = Snapshot {
        backends: vec![aeg_ir::BackendCluster {
            ai_service: None,
            token_policy: None,
            name: format!("auth:{auth_port}"),
            namespace: "default".to_string(),
            protocol: "HTTPS".to_string(),
            endpoints: vec![aeg_ir::BackendEndpoint {
                address: "127.0.0.1".to_string(),
                port: auth_port,
                healthy: true,
            }],
...
fn external_auth_with_backend_tls_and_session_persistence_combo_snapshot() {
    use aeg_ir::{BackendPolicy, BackendTlsValidation, LoadBalancingPolicy, SessionPersistence, Snapshot};

    let snapshot = Snapshot {
        backends: vec![
            aeg_ir::BackendCluster {
                ai_service: None,
                token_policy: None,
                name: format!("backend:8080"),
                namespace: "default".to_string(),
                protocol: "HTTP".to_string(),
                endpoints: vec![aeg_ir::BackendEndpoint {
            ...
            m.insert(
                ...
                BackendPolicy {
                    ...
                    load_balancing: Some(LoadBalancingPolicy {
                        ...
                        consistent_hash: Some(aeg_ir::ConsistentHashPolicy {
