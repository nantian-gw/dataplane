...
pub(super) use self::assets::{
    cleanup_unused_tls_assets_in_dir, materialize_runtime_plan, materialize_tls_assets_in_dir,
    referenced_tls_asset_prefixes, unique_asset_dir_name, TlsAssetWriteStats,
};
#[cfg(test)]
...
fn frontend_validation_bundle(validation: &aeg_ir::FrontendValidation) -> Option<String> {
...
fn frontend_validation_mode(validation: &aeg_ir::FrontendValidation) -> Option<String> {
...
pub(super) fn ordered_tls_identity_candidates<'a>(
    ...
    for (index, identity) in identities.iter().enumerate() {
        if let Some(server_name) = server_name {
            if let Some(rank) = tls_identity_match_rank(identity, server_name) {
