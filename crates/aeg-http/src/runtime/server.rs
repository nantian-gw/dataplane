...
fn add_tls_http_service(
    ...
    for listener in tls_listeners {
        ...
        if let Some(client_ca_path) = client_ca_path.as_deref() {
            ...
            settings
                .set_session_id_context(b"aeg-http")
...
fn build_gateway_proxy(
    ...
    experimental: aeg_config::ExperimentalConfig,
) -> GatewayProxy {
...
fn build_ai_filter(
    snapshot: &SharedSnapshot,
    wasm_filter: Option<Arc<aeg_ai::wasm_filter::WasmPluginFilter>>,
) -> Option<Arc<aeg_ai::filter::AIGatewayFilter>> {
    use aeg_ai::filter::AIGatewayFilter;
    use aeg_ai::format::anthropic::AnthropicAdapter;
    use aeg_ai::format::ollama::OllamaAdapter;
    use aeg_ai::format::openai::OpenAIAdapter;
    use aeg_ai::format::AdapterRegistry;
    use aeg_ai::observability::metrics::AIMetrics;
    use aeg_ai::ratelimit::{RateLimitConfig, TokenRateLimiter};
...
fn build_wasm_filter(
    snapshot: &SharedSnapshot,
) -> Option<Arc<aeg_ai::wasm_filter::WasmPluginFilter>> {
    use aeg_ai::wasm_filter::WasmPluginFilter;
    use aeg_wasm::plugin::{WasmHook, WasmSandboxConfig};
    use aeg_wasm::PluginManager;

    let engine = match aeg_wasm::engine::create_engine() {
        Ok(engine) => engine,
        Err(e) => {
            tracing::warn!(
                target: "wasm",
